...
def cmd_export_lists(a):
    with conn() as c:
        ...
        with open(csvp, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["addr", "chain", "status", "reason", "updated_at"])
...
def cmd_rounds(a):
    ...
    dec = 9
    try:
        sup = rpc.get_token_supply(a.mint)
        dec = int(sup.get("value", {}).get("decimals", 9))
    except:
        pass
    t0 = None
    try:
        t0 = estimate_t0(rpc, a.mint, sample_holders=10)
    except:
        pass
    with conn() as c:
        if a.addr:
            ...
            cur = c.execute(
                "SELECT addr FROM lists WHERE status='WHITE' ORDER BY updated_at DESC LIMIT ?;", (a.limit,)
            )
    ...
    with open(out, "w", newline="") as f:
        ...
        for addr in addr_list:
            ...
            for r in rs:
                w.writerow([
                    addr, r.get("entry_ts"), r.get("exit_ts"), r.get("hold_s"),
...
def cmd_score_white(a):
    ...
    rows = score_white_for_mint(
        rpc, a.mint, addrs,
        price_url=a.price_url, price_key=a.price_key,
        t0=None, decimals=dec, sleep_ms=a.sleep_ms
    )
    ...
    export_csv(rows, csvp)
    print(f"[OK] CSV  -> {csvp}", flush=True)
    if a.topk > 0:
        txtp = f"data/exports/white_top_{a.mint[:6]}_{ts}.txt"
        export_txt_addrs(rows, txtp, a.topk)
        print(f"[OK] TOPK -> {txtp}", flush=True)
...
def cmd_score_watch(a):
    ...
    rows = score_watch_for_mint(
        rpc, a.mint, addrs,
        price_url=a.price_url, price_key=a.price_key,
        t0=None, decimals=dec, sleep_ms=a.sleep_ms,
        require_activity=a.require_activity
    )
...
def main():
    ...
    p.add_argument("--amm", default="raydium")
    p.add_argument("--base", default="USDC")
    p.add_argument("--quote", default="SOL")
    ...
    p.add_argument("--kind", choices=["WHITE", "WATCH", "BLACK"], required=True)
    p.set_defaults(func=cmd_export_lists)

    p = sub.add_parser("t0")
    p.add_argument("--mint", required=True)
    p.add_argument("--sample", type=int, default=15)
    p.set_defaults(func=cmd_t0)

    p = sub.add_parser("rounds")
    p.add_argument("--mint", required=True)
    p.add_argument("--addr")
    p.add_argument("--limit", type=int, default=50)
    p.add_argument("--price_url")
    p.add_argument("--price_key")
    ...
    p.add_argument("--pos-expect", action="store_true")
    p.add_argument("--topk", type=int, default=50)
    p.add_argument("--sleep-ms", type=int, default=0)
    p.add_argument("--price_url")
    p.add_argument("--price_key")
    p.set_defaults(func=cmd_score_white)
    ...
    p.add_argument("--price_url")
    p.add_argument("--price_key")
    p.set_defaults(func=cmd_score_watch)
